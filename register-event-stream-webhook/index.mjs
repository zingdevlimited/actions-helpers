...
const eventTypes = EVENT_TYPES.split("\n")
  ...
  .map(([type, version]) => ({
    type: type.trim(),
    schema_version: Number.parseInt(version.trim()),
  }));
...
const existing = sinkList.find(
  (s) => s.sink_configuration?.destination === SINK_WEBHOOK_URL
);
...
if (existing) {
  ...
  console.log(
    `Created Event Sink '${SINK_DESCRIPTION}': ${SINK_WEBHOOK_METHOD} ${SINK_WEBHOOK_URL}`
  );
  currentSink = sinkCreateResponse.body;
}

const subscriptionListResponse = await asyncTwilioRequest(
  `${eventsUrl}/Subscriptions?SinkSid=${currentSink.sid}`,
  "GET"
);
...
if (subscriptionList.length) {
  ...
  const eventsToAdd = eventTypes
    .filter((e) => !subscribedEventTypes.some((s) => e.type === s.type))
    .map(async (e) => {
      await asyncTwilioRequest(
        ...
        new URLSearchParams({
          Type: e.type,
          SchemaVersion: e.schema_version.toString(),
        })
      );
      console.log(
        `Added Event to Subscription '${SUBSCRIPTION_DESCRIPTION}': ${e.type}/v${e.schema_version}`
      );
    });

  const eventsToUpdate = eventTypes
    .filter((e) => {
    ...
    .map(async (e) => {
      ...
      await asyncTwilioRequest(
        ...
        "POST",
        new URLSearchParams({ SchemaVersion: e.schema_version.toString() })
      );
      console.log(
        `Updated Event in Subscription '${SUBSCRIPTION_DESCRIPTION}': ${e.type} - v${existing.schema_version} -> v${e.schema_version}`
      );
    });

  const eventsToDelete = subscribedEventTypes
    .filter((s) => !eventTypes.some((e) => s.type === e.type))
    .map(async (s) => {
      await asyncTwilioRequest(
      ...
      console.log(
        `Deleted Event from Subscription '${SUBSCRIPTION_DESCRIPTION}': ${s.type}`
      );
    });
  ...
  console.log(
    `Created Subscription '${SUBSCRIPTION_DESCRIPTION}' with events ${eventTypes.map(
      (e) => e.type
    )}`
  );
...
if (GITHUB_OUTPUT) {
  ...
  appendFileSync(
    GITHUB_OUTPUT,
    `SUBSCRIPTION_SID=${currentSubscription.sid}\n`,
    "utf8"
  );
}
