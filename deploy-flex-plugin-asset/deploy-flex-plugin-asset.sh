...
function prepareService {
  local serviceName includeCredentials friendlyName uiEditable

  serviceName=$1
  [[ "$2" == "true" ]] && includeCredentials="True" || includeCredentials="False"
  [[ -n "$3" ]] && friendlyName="$3" || friendlyName="$serviceName"
  [[ "$4" == "false" ]] && uiEditable="False" || uiEditable="True"
  ...
  if [ -z "$serviceResponse" ]; then
    serviceResponse=$(curl -sX POST "https://serverless.twilio.com/v1/Services" \
      ...
      --data-urlencode "UiEditable=$uiEditable" \
...
function generateSuffix {
  ...
  if [ "$(echo "$suffixList" | jq "any(index(\"$generatedSuffix\"))")" == "true" ]; then
    echo "Suffix collision, regenerating suffix..." >&2
...
serviceSid=$(prepareService "default" "true" "Flex Plugins Service (Autogenerated) - Do Not Delete" "false") || exit 1
...
if [ -z "$pluginEnvironment" ]; then
  echo "Environment with unique name '$pluginName' not found. Creating new..." >&2
